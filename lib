...
function DiscordLib:Window(text)
	...
	if input.KeyCode == Enum.KeyCode.V then -- the button lol
 		if opened == true then
			MainFrame:TweenSize(UDim2.new(0, 681, 0, 0), "InOut", "Linear", 0.2, true)
			wait(0.2)
			opened = false
		elseif opened == false then
			MainFrame:TweenSize(UDim2.new(0, 681, 0, 396), "InOut", "Linear", 0.2, true)
			wait()
			opened = true
		end
	end
